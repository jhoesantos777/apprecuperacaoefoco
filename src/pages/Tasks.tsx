...
const Tasks = () => {
  ...
  const [expandedCategories, setExpandedCategories] = useState<string[]>(
    // Por padrão, exibir todas as categorias expandidas
    TaskCategories.map(cat => cat.id)
  );
  ...
  const getCategoryMaxPoints = (categoryId: string) => {
    ...
    const categoryTasks = tasks.filter(task => task.category_id === categoryId) || [];
    return categoryTasks.reduce((acc, task) => acc + task.points, 0);
  };
  ...
  const getBarColor = (points: number, maxPoints: number) => {
    if (maxPoints === 0) return 'bg-gray-300';
    const percentage = (points / maxPoints) * 100;
  ...
  useEffect(() => {
    if (!tasksLoading && !completionsLoading && tasks && completions) {
      checkAllTasksCompleted();
    }
  }, [tasksLoading, completionsLoading, tasks, completions]);
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              ...
              <div className="w-full sm:w-48 h-3 bg-slate-700 rounded-full overflow-hidden relative">
                <motion.div 
                  ...
                  initial={{ width: 0 }}
                  animate={{ 
                    width: isLoading ? '100%' : `${maxPoints > 0 ? (totalPoints / maxPoints) * 100 : 0}%`
                  }}
                  transition={{ 
            ...
            <AnimatePresence>
              {expandedCategories.includes(category.id) && (
                <motion.div 
                  ...
                  transition={{ duration: 0.3 }}
                  className={`mt-2 p-3 rounded-xl ${category.cardBg} backdrop-blur-sm border ${category.borderColor}`}
                >
                  {tasks
                  ...
                  {tasks.filter(task => task.category_id === category.id).length === 0 && (
                    <div className="text-center py-4 text-gray-500">
                      Nenhuma tarefa disponível nesta categoria.
                    </div>
                  )}
                </motion.div>
              )}
