...
// Função para obter o devocional do dia conforme fornecido pelo usuário
export function getDevocionalDoDia() {
  const hoje = new Date();
  const diaDoAno = Math.floor(
    (Date.UTC(hoje.getFullYear(), hoje.getMonth(), hoje.getDate()) -
     Date.UTC(hoje.getFullYear(), 0, 1)) / 86400000
  ) + 1;

  const index = (diaDoAno - 1) % dailyVerses.length; // garante que não ultrapassa o tamanho do array
